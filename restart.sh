...
docker run -it --name redis-master -v ${PWD}/master/redis.conf:/etc/redis.conf \
  ...
  -d -p ${MASTER_PORT}:6379 --net mynetwork --ip 172.10.0.2 redis redis-server /etc/redis.conf
...
docker run -it --name redis-slave -v ${PWD}/slave/redis.conf:/etc/redis.conf \
  ...
  -d -p ${NODE_PORT}:6379 --net mynetwork --ip 172.10.0.3 redis redis-server /etc/redis.conf
...
docker run -it --name redis-sentinel -v ${PWD}/sentinel/sentinel.conf:/etc/sentinel.conf \
  ...
  -d -p ${SENTINEL_PORT}:26379 --net mynetwork --ip 172.10.0.4 redis redis-sentinel /etc/sentinel.conf
