...
export NODE_PORT=10002
export SENTINEL_PORT=10003
...
docker stop redis-sentinel redis-slave redis-master
...
docker run -itd --name redis-sentinel -v ${PWD}/sentinel/sentinel.conf:/etc/sentinel.conf -v /data/redis/${SENTINEL_PORT}:/data -p ${SENTINEL_PORT}:26379 --net mynetwork --restart=always --ip 172.10.0.4 redis "redis-sentinel /etc/sentinel.conf"
